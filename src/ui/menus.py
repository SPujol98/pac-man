...
class MainMenu:
    """Manages the logic, keyboard navigation, and rendering of the Main Menu"""
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        for i, option in enumerate(self.options):
            ...
            opt_surf = self.option_font.render(f"{prefix}{option}", True, color)
            opt_rect = opt_surf.get_rect(center=(self.width // 2, start_y + i * 50))
            surface.blit(opt_surf, opt_rect)

        info_font = pygame.font.SysFont("Arial", 24)
        info_surf = info_font.render("© 2026 spujol-s/sasanche. All rights reserved.", True, (150, 150, 150))
        info_rect = info_surf.get_rect(center=(self.width // 2, self.height - 40))
