...
class PauseMenu(BaseScreen):
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        card_rect = pygame.Rect((self.width - card_w) // 2, 170, card_w, card_h)
        pygame.draw.rect(surface, self.COLOR_CARD_BG, card_rect, border_radius=8)
        pygame.draw.rect(surface, self.COLOR_BORDER, card_rect, width=2, border_radius=8)
        ...
        for i, option in enumerate(self.options):
            ...
            row_rect = pygame.Rect(card_rect.left + 20, option_y - 8, card_w - 40, 40)

            if is_selected:
                pygame.draw.rect(surface, self.COLOR_HIGHLIGHT, row_rect, border_radius=6)
                pygame.draw.rect(surface, self.COLOR_SELECTED, row_rect, width=1, border_radius=6)
                pygame.draw.circle(surface, self.COLOR_TITLE, (row_rect.left + 25, row_rect.centery), 6)
        ...
        info_surf = self.footer_font.render("Press P or ESC to Resume", True, (160, 160, 170))
        info_rect = info_surf.get_rect(center=(self.width // 2, card_rect.bottom + 35))
